...
@RestController
@RequestMapping({"/api/mentor/room-quizzes", "/api/creator/room-quizzes"})
public class RoomQuizController {
    ...
    @PostMapping("/full")
    public RoomQuiz createFullQuiz(@RequestBody CreateFullQuizRequest request) {
        return roomQuizService.createFullQuiz(request);
    }

    @PostMapping("/{quizId}/submit")
    public RoomQuizSubmission submitQuiz(
            @PathVariable String quizId,
            @RequestBody SubmitQuizRequest request) {
        return roomQuizService.submitQuiz(quizId, request);
    }
